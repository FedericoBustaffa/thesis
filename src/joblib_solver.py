...
def parallel_task(crossoverator, mutator, evaluator, couples):
...
class JoblibGeneticSolver(GeneticSolver):
    def __init__(
        self,
        population_size: int,
        generation_func,
        ...
        self.__executor = joblib.Parallel(n_jobs=workers_num, batch_size="auto")
    ...
    def run(self, max_generations: int):
        ...
        for g in range(max_generations):
            ...
            chunksize = math.ceil(len(couples) / self.__executor.n_jobs)

            results = self.__executor(
                joblib.delayed(parallel_task)(
                    self._crossoverator,
                    self._mutator,
                    self._evaluator,
                    couples[i * chunksize : i * chunksize + chunksize],
                )
                for i in range(self.__executor.n_jobs)
            )

            offsprings = []
            offsprings_scores = []
            for r in results:
                offsprings_chunk, offsprings_scores_chunk = r
